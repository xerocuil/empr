...
class GameForm(FlaskForm):
    ...
    esrb = SelectField('ESRB', choices=ESRB_RATINGS, validators=[Length(max=4), Optional()])
    favorite = BooleanField()
    filename = StringField('File Name', validators=[DataRequired('File Name required.'), Length(max=128)])
    ...
    platform_id = IntegerField('Platform', validators=[DataRequired('Platform required.')])
    players = IntegerField('Player(s)', validators=[NumberRange(max=64), Optional()])
    ...
    region = SelectField(choices=REGIONS, validators=[Length(max=2), Optional()])
    ...
    year = IntegerField(
        validators=[NumberRange(min=1940, max=2999, message='Year entered is out of sanity range.'), Optional()])
